class TipperBot
  ...
  def post_tip_on_twitter(fromUser, toUser, txid, tweet_id)
    message = "@#{fromUser["TwitterUsername"]} just tipped @#{toUser["TwitterUsername"]} #{B::TIP_AMOUNT_UBTC.to_i}μBTC for twitter.com/#{toUser["TwitterUsername"]}/status/#{tweet_id} transaction: downloadtipper.com/tip/#{txid}"
    begin
